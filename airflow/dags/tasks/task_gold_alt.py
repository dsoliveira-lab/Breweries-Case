...
def brewery_etl_gold(silver_bucket, gold_bucket, endpoint_url, access_key, secret_key):
    ...
    def read_silver_data(minio_client, silver_bucket):
        try:
            silver_object = minio_client.get_object(Bucket=silver_bucket, Key='ds_silver.parquet')
    ...
    try:
        ...
        minio_client.put_object(Bucket=gold_bucket, Key='ds_gold.parquet', Body=parquet_buffer.getvalue())
            ...
            with connection.cursor() as cursor:
                ...
                for index, row in gold_df.iterrows():
                    try:
                        cursor.execute(
                            """
                            INSERT INTO ds_gold (brewery_type, city, state_province, country, brewery_count)
                            VALUES (%s, %s, %s, %s, %s)
